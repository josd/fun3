...
def query(final_ctu):
    ...
    if Iri('http://example.org/robocop') == Iri('http://example.org/robocop'):
        rule_0(Iri('http://example.org/robocop'), lambda a_0: final_ctu())
...
query(lambda: print(Triple(Iri('http://example.org/robocop'), Iri('http://example.org/part'), Iri('http://example.org/robocop')).instantiate({})))