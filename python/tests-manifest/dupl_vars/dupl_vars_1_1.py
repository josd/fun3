...
def query(final_ctu):
    data.find(Iri('http://example.org/robocop'), Iri('http://example.org/part'), Iri('http://example.org/terminator'), lambda s, p, o: final_ctu())
    if Iri('http://example.org/robocop') == Iri('http://example.org/terminator'):
        rule_0(Iri('http://example.org/terminator'), lambda a_0: final_ctu())
...
query(lambda: print(Triple(Iri('http://example.org/robocop'), Iri('http://example.org/part'), Iri('http://example.org/terminator')).instantiate({})))