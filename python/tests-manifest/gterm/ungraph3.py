...
def rule_0(a_2, t_3, final_ctu):
    ...
    rule_1(a_2, GraphTerm(triples=[Triple(Var('part_4'), NS.rdf['type'], t_3)]), lambda q_5, data_6: final_ctu(q_5, data_6[0][2]))

def rule_1(q_5, data_6, final_ctu):
    data.find(q_5, Iri('http://example.org/hasParts'), data_6, lambda s, p, o: final_ctu(s, o))
query(ANY, ANY, lambda a_0, t_1: print(Triple(Var('a_0'), Iri('http://example.org/partType'), Var('t_1')).instantiate({'a_0': a_0, 't_1': t_1})))