...
def query(a_0, final_ctu):
    ...
    rule_0(a_0, Iri('http://example.org/machine'), lambda a_1, t_2: final_ctu(a_1))
...
def rule_0(a_1, t_2, final_ctu):
    ...
    rule_1(a_1, GraphTerm(triples=[Triple(Var('part_3'), NS.rdf['type'], t_2)]), lambda q_4, data_5: final_ctu(q_4, data_5[0][2]))

def rule_1(q_4, data_5, final_ctu):
    data.find(q_4, Iri('http://example.org/hasParts'), data_5, lambda s, p, o: final_ctu(s, o))
query(ANY, lambda a_0: print(Triple(Var('a_0'), Iri('http://example.org/partType'), Iri('http://example.org/machine')).instantiate({'a_0': a_0})))