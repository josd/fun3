...
def query(final_ctu):
    ...
    rule_0(Literal(3, NS.xsd['int']), Literal(4, NS.xsd['int']), Literal(7, NS.xsd['int']), lambda x_0, y_1, r_2: final_ctu())

def rule_0(x_0, y_1, r_2, final_ctu):
    math_sum(Collection([x_0, y_1]), r_2, lambda s, o: final_ctu(s[0], s[1], o))
query(lambda: print(Triple(Collection([Literal(3, NS.xsd['int']), Literal(4, NS.xsd['int'])]), Iri('http://example.org/result'), Literal(7, NS.xsd['int'])).instantiate({})))