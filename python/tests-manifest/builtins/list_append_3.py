...
def query(x, final_ctu):
    data.find(Iri('http://example.org/result'), Iri('http://example.org/is'), Collection([x]), lambda s, p, o: final_ctu(o[0]))
...
query(ANY, lambda x: print(Triple(Iri('http://example.org/result'), Iri('http://example.org/is'), Collection([Var('x')])).instantiate({'x': x})))