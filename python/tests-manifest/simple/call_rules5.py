...
def rule_0(p_0, t_1, final_ctu):
    ...
    rule_1(p_0, t_1, lambda p_3, t_4: rule_0_1(p_3, t_4, final_ctu))
    if t_1 == Iri('http://example.org/Greek'):
        rule_2(p_0, lambda p_5: rule_0_1(p_5, Iri('http://example.org/Greek'), final_ctu))
...
def rule_1(p_3, t_4, final_ctu):
    data.find(p_3, Iri('http://example.org/describedAs'), t_4, lambda s, p, o: final_ctu(s, o))

def rule_2(p_5, final_ctu):
    data.find(p_5, Iri('http://example.org/name'), Literal('Socrates', NS.xsd['string']), lambda s, p, o: final_ctu(s))
