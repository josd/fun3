...
def rule_0(p_1, final_ctu):
    ...
    rule_1(p_1, lambda p_3: rule_0_1(p_3, final_ctu))
...
def rule_1(p_3, final_ctu):
    data.find(p_3, Iri('http://example.org/ability'), Iri('http://example.org/think'), lambda s, p, o: final_ctu(s))

def rule_2(pe_4, final_ctu):
