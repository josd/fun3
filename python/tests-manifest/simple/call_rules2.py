...
def query(x_0, final_ctu):
    ...
    rule_1(x_0, Iri('http://example.org/Canadian'), lambda pe_3, ty_4: final_ctu(pe_3))

def rule_0(p_1, final_ctu):
    data.find(p_1, NS.rdf['type'], Iri('http://example.org/Person'), lambda s, p, o: rule_0_1(s, final_ctu))
    rule_1(p_1, Iri('http://example.org/Person'), lambda pe_3, ty_4: rule_0_1(pe_3, final_ctu))
...
def rule_1(pe_3, ty_4, final_ctu):
    data.find(pe_3, Iri('http://example.org/describedAs'), ty_4, lambda s, p, o: final_ctu(s, o))
